...
for i = 1:iter
    ...
    if i > 3
        mz = (trace(kron(Iblock, Sz)*rho))/syslenght;
        fprintf('%s\n%f', 'mz:', mz);
    end

end
disp(['Le dimensioni di rho sono: ', num2str(size(rho))]);
...
disp(['Le dimensioni di Iblock tensor Sz sono: ', num2str(size(kron(Iblock, Sz)))]);
fclose(fileID);
