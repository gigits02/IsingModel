...
/*
    FINITE-SYSTEM - DMRG METHODS FOR BOSE-HUBBARD MODEL IN 1D;
...
const int ITER = 50; // MAX NUMBER OF ITERATIONS;
...
{   
    ...
    for(int i=1;i<ITER+1;i++)
    {
        ...
        // TRACCIA PARZIALE TRAMITE RESHAPING;
...
void SWEEPING_DX(BlockStructure& STRUCTURE, SparseMatrix<double>& b, SparseMatrix<double>& b_dagger, SparseMatrix<double>& n, SparseMatrix<double>& I, double mu)
{
    ...
    for(int i=1;i<ITER+1;i++)
    {
        ...
        outputFile << 2*ITER+2 << " " << Energy << " " << En_Bond << " " << En_Density << " " << Tronc_error << " " << ent << endl;
...
int main()
{
    ...
    cout << "Simulazione per Nmax = " << Nmax <<", J = " << J << ", mu = " << mu << endl;
    ...
    // INIZIALIZZAZIONE BLOCCHI ASSUMENDO A--a--b--B CON b--B SPECULARE AD A--a;
    BlockStructure STRUCTURE = initializeBlockStructure(b,b_dagger,n,I,H);

    DMRG(STRUCTURE, b, b_dagger, n, I, mu, Nmax);
    SWEEPING_DX(STRUCTURE, b, b_dagger, n, I, mu);
    SWEEPING_SX(mu);
            
    cout <<"Ho salvato nel file di output ... " << endl;
    cout << "LENGHT" << " " << "Energy" << " " << "En_Bond" << " " << "En_Density" << " " << "TruncErr" << " " << "Ent" << "n_medio" << endl;
